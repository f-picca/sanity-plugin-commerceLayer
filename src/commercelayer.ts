...
const C11R_INTEGRATION_SECRET = process.env.SANITY_STUDIO_C11R_INTEGRATION_SECRET!
const C11R_INTEGRATION_SLUG = process.env.SANITY_STUDIO_C11R_INTEGRATION_SLUG!
const C11R_DASHBOARD_BASE_URL = process.env.SANITY_STUDIO_C11R_DASHBOARD_BASE_URL!
const C11R_DASHBOARD_SALES_CHANNEL_ID = process.env.SANITY_STUDIO_C11R_SALES_CHANNEL_ID!
  ...
  try {
    if (result.length > 0) {
      ...
      const updatedSku = await cl.skus.update({
        ...
        shipping_category: cl.shipping_categories.relationship(_sku.shippingCategory.code),
      })
    ...
    return {
      operation: result.length > 0 ? SyncOperation.update : SyncOperation.create,
      success: false,
      message: `Failed to ${
...
export const getSkuCommerceData = async (_sku: string): Promise<SkuCommerceData | undefined> => {
  const cl = getClClient(await getToken())
  const sku = await cl.skus.retrieve(_sku, {
    include: ['prices', 'prices.price_list', 'stock_items', 'stock_items.stock_location'],
  })
...
const getShoppingLinks = async (sku: ClSku): Promise<Record<string, string>> => {
  ...
  const validLinks = links.filter((link): link is Record<string, string> => link !== null)
...
export const getShippingCategories = async (): Promise<ListResponse<ShippingCategory>> => {
  const cl = getClClient(await getToken())
...
export const getClSkuDashboardLink = (skuId: Sku): string => {
  const clAppSkuUrl = `${C11R_DASHBOARD_BASE_URL + C11R_INTEGRATION_SLUG}/apps/skus/list/${skuId}`
